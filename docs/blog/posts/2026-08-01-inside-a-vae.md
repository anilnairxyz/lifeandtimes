...
An [earlier post](2024-07-vaes.md) worked through the theory of the variational auto-encoder: the latent-variable motivation, the intractable marginal likelihood, and the ELBO with its two terms — a reconstruction term and a KL term that pulls the approximate posterior towards the prior. This is the companion to that post. Rather than more equations, it is what happens when you actually build the thing, deliberately cripple it down to a **two-dimensional** latent space, and look at what it learned.
