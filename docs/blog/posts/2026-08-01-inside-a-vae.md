...
We next use a weight, $\beta$ on the KL term, which acts as a dial on the ELBO, tuning it between reconstruction and regularisation. Holding $L = 32$ and sweeping $\beta$ we trace out the **rate–distortion curve**{: .rust}, plotting rate $R$ (the KL, the information pushed through the latent) against distortion $D$ (the reconstruction cost), both in nats.
