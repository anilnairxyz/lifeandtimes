...
Modern language models can converse fluently, but trustworthy answers to tax questions require more than eloquence. This post outlines a pragmatic, production-oriented approach to building a tax copilot that answers questions and performs calculations reliably. It is based on the approach we implemented for Earnr, a finance and tax app for the self employed indivuals and their accountants in the UK. It focuses on the technology and the philosophy behind the system, not any specific implementation details.
...
- Perform calculations (e.g., capital gains tax, take-home pay) reliably and explain the math for the correct tax year.
...
flowchart TB
  ...
  subgraph RETR[Retrieval]
    ...
    ENS --> WDEC{Web needed?}
    WDEC -- No --> CTX[Context Builder]
    WDEC -- Google Search --> G[Google Search]
    WDEC -- Web LLM --> PPLX[Web LLM]
    G --> CTX
    PPLX --> CTX
  end
...
- Ask the model to answer using that context and cite references where possible.
...
HYDE improves retrieval by embedding not just the question but also a hypothetical answer for vector search.

- Rephrase the question to be standalone (remove chat dependencies).
- Generate a short hypothetical answer.
- Retrieve documents similar to this hypothetical answer.

Documents in “answer space” are often more coherent than documents matched to short, ambiguous questions that a typical customer is likely to ask. HYDE is a technique from research, implemented here using standard LLMs for the rephrase/answer steps
...
Sometimes the best answer is on the open web (e.g., brand‑new guidance relevant for the next tax year). Here we:
...
- Use web answers when the internal knowledge is suspect/nsufficient for a particular context.
...
- Instruct the model to explain results from tool outputs only, without resorting to “freehand” math.
...
???+ info "Notes"
    ...
    - “Browsing LLM” denotes an API that integrates search + page fetching during generation like Perplexity
