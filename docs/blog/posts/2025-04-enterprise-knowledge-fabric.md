...
## 2. Technical Deep Dive: Embeddings and Semantic Search

The "Search" component of RAG is where the battle for accuracy is won or lost. In EKF, we treat retrieval as a multi-stage optimisation problem.

### Data Preparation and Chunking

Before data is vectorised, it must be "chunked." We found that large documents lose nuance when embedded as a single block. We implemented a **sliding window approach**:
...
### Vectorisation with SentenceTransformers

We utilise **SentenceTransformers** to map text into a high-dimensional dense vector space. This allows the system to understand that a query for _"How do I reset my credentials?"_ is semantically similar to a policy document titled _"Password Recovery Procedures."_

To ensure mathematical consistency during similarity search, we perform **unit-length normalisation** on all vectors. This allows us to use the **Inner Product** as a proxy for **Cosine Similarity**, which is computationally more efficient.
...
## 3. Global Reach: Multi-Language Support
